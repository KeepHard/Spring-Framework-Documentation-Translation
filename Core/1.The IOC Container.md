...
##### 使用实例工厂方法实例化
与通过静态工厂方法实例化类似，通过实例工厂方法实例化是调用一个容器中已经存在的bean的非静态方法来创建bean。要使用这个机制，class属性置空，factory-bean属性指定当前（或父母或祖先）容器中包含实例方法的bean的名称。这个实例方法就是用来创建对象的。将factory-method属性设置为该实例方法的名字。下面的例子展示了如何配置这样的bean：
